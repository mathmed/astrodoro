"""Ícones vetoriais embutidos, tingidos com a cor do tema.

Por que não emoji: emoji é bitmap colorido e não pode ser tingido. No modo
noturno você teria pontos azuis e verdes brilhando na tela, exatamente o que a
rampa vermelha da imagem existe para evitar. Todo ícone aqui é traço
monocromático que recebe a cor do tema, então no noturno fica vermelho junto com
o resto.

Cada ícone é um SVG de 24x24 escrito à mão. Renderizado em 2x para ficar nítido
em tela retina, e cacheado por (nome, cor, tamanho).
"""
...
# alças dos sliders do ícone "adjust", desenhadas cheias por cima do traço
_EXTRA_FILL = {
    "adjust": 'M9 7a1.9 1.9 0 1 0 0-.02z M15 12a1.9 1.9 0 1 0 0-.02z'
              ' M7 17a1.9 1.9 0 1 0 0-.02z',
}
...
def _svg(name: str, color: str, width: float) -> str:
    ...
    if name in _EXTRA_FILL:
        parts.append(f'<circle cx="9" cy="7" r="2.1" fill="{color}"/>'
                     f'<circle cx="15" cy="12" r="2.1" fill="{color}"/>'
                     f'<circle cx="7" cy="17" r="2.1" fill="{color}"/>')
    if not parts:
