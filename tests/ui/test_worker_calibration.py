...
from astrodoro.drivers import Bayer, Geometry
...
class _Source:
    ...
    def open(self):
        return {
            "width": W,
            "height": H,
            "bin": self.cam.geometry.bin,
            "full_scale": SCALE,
            "bayer": self.cam.bayer,
            "live": True,
        }
...
def test_a_flat_is_corrected_by_the_bias_not_by_a_five_second_dark(worker, tmp_path):
    worker.cfg.bias_path = str(_master(tmp_path, "bias", PEDESTAL))
    worker._load_bias()
...
def test_a_dark_of_the_flats_own_exposure_does_qualify(worker, tmp_path):
    worker.src.cam.exposure = 0.05
    worker.cfg.dark_path = str(_master(tmp_path, "dark", PEDESTAL, exposure=0.05))
    worker._load_dark()
...
def test_the_recorded_flat_carries_the_vignetting_it_measured(worker, tmp_path):
    ...
    assert worker._flat.min() == pytest.approx(
        0.5 / float(np.median(worker.src.vignette)), rel=0.03
    )


def test_the_session_applies_bias_and_flat_to_the_frame_it_emits(worker, tmp_path):
    from astrodoro.core import masters

    vig = _vignette(depth=0.6)
    worker.cfg.bias_path = str(_master(tmp_path, "bias", PEDESTAL))
    worker._load_bias()
    worker.cfg.flat_path = str(
        masters.write(
            tmp_path,
            "flat",
            (20000.0 * vig).astype(np.float32),
            masters.Setup(
                gain=250,
                offset=20,
                exposure=0.05,
...
def test_the_headless_session_refuses_a_master_of_another_bin(tmp_path):
    ...
    args = SimpleNamespace(
        bias=None,
        dark=None,
        flat=str(_master(tmp_path, "flat", 20000.0, shape=(H // 2, W // 2), bin=4)),
    )
...
def test_a_flat_with_no_signal_is_dropped_headless_too(tmp_path):
    ...
    args = SimpleNamespace(
        bias=None, dark=None, flat=str(_master(tmp_path, "flat", 0.0))
    )
