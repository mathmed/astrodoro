...
def test_arrow_hidden_outside_realign(window):
    ...
    assert not window.image.realign_arrow.isVisible()
...
def test_arrow_shown_pointing_at_a_correction(window):
    ...
    assert window.image.realign_arrow.isVisible()
...
def test_arrow_reports_on_target_below_the_threshold(window):
    ...
    window.on_frame(rgb, None, _cfa(), _stats(realign=info, realigning=True))
    assert window.image.realign_arrow.isVisible()
    assert window.image.realign_arrow._pointing is False
...
def test_arrow_flags_a_failed_registration_with_the_specific_reason(window):
    ...
    info = {
        ...
        "distance": 0.0,
        "rotation_deg": 0.0,
        "reason": "only 2 stars detected",
    }
    window.on_frame(rgb, None, _cfa(), _stats(realign=info, realigning=True))
    assert window.image.realign_arrow.isVisible()
    assert window.image.realign_arrow._pointing is False
    assert window.image.realign_arrow._label == "only 2 stars detected"


def test_toggling_the_button_flips_label_and_hides_the_arrow(window):
    window.worker = _FakeWorker()
    window.integrate.btn_realign.setChecked(True)
    assert "Resume" in window.integrate.btn_realign.text()
    ...
    window.integrate.btn_realign.setChecked(False)
    assert "Pause & realign" in window.integrate.btn_realign.text()
    assert not window.image.realign_arrow.isVisible()
...
def test_checking_realign_before_a_session_exists_reverts_and_logs(window):
    ...
    window.integrate.btn_realign.setChecked(True)
    assert not window.integrate.btn_realign.isChecked()
