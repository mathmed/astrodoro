...
def test_a_master_of_the_wrong_shape_is_ignored_not_broadcast():
    raw = _light()
    for kw in (
        {"dark": np.zeros((H + 2, W), np.float32)},
...
def test_flat_master_subtracts_the_pedestal_before_normalising():
    ...
    assert 1.0 - residual.min() / residual.max() == pytest.approx(0.024, abs=0.004)
...
def _hdr(kind, **kw):
    s = masters.Setup(
        gain=kw.pop("gain", 250),
        offset=kw.pop("offset", 20),
        exposure=kw.pop("exposure", 5.0),
        bin=2,
...
def test_the_offset_matters_for_a_bias_and_not_for_a_flat():
    ...
    assert any(
        "offset" in m for m in masters.mismatch("bias", _hdr("bias", offset=8), want)
    )
...
def test_exposure_and_temperature_are_a_darks_business_only():
    want = masters.Setup(gain=250, offset=20, exposure=5.0, bin=2, temperature=-10.0)
    ...
    assert masters.mismatch("dark", _hdr("dark", exposure=5.0, temperature=-3.0), want)
