...
def _sequence(n=24):
    ...
    for k in range(n):
        ...
        rot = k * 0.045
        if k >= 12:
            drift = drift + np.array([180.0, -140.0])
            rot += 1.3
...
def test_stack_survives_a_platform_reset_and_gains_snr():
    ...
    st = LiveStacker(
        (H, W), channels=1, ref_refresh=6, min_matched=8, max_rms=2.0, sigma_clip=3.0
    )
...
def test_preview_alignment_reports_the_offset_without_mutating_state():
    ...
    st = LiveStacker((H, W), channels=1, ref_refresh=6, min_matched=8, max_rms=2.0)
...
def test_sigma_clipping_removes_the_satellite():
    ...
    diagonal = np.array(
        [
            stack_adu[470 - int(u * 430), 30 + int(u * 440)]
