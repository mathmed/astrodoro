...
from ..drivers import CameraDevice, CameraError, ImgType, camera, list_cameras
from ..settings import Settings


def pick() -> CameraDevice:
    cams = list_cameras()
    if not cams:
...
def stats(a: np.ndarray, full: int = 16383) -> str:
    ...
    return (
        f"min={a.min()} max={a.max()} median={np.median(a):.0f} "
...
def cmd_grab(args) -> None:
    ...
    with pick() as cam:
        ...
        if cam.supports_cooler:
            print(
                f"sensor at {cam.temperature:.1f} C, "
        ...
        for n in range(args.frames):
            ...
            print(
                f"  frame {n + 1}/{args.frames}  {dt * 1000:7.1f} ms  "
...
def save_fits(path: Path, frame: np.ndarray, cam: CameraDevice) -> None:
...
def cmd_bench(args) -> None:
    with pick() as cam:
        ...
        if times:
            t = np.array(times)
            print(
                f"n={len(t)}  mean={t.mean() * 1000:.1f} ms  "
                f"min={t.min() * 1000:.1f}  max={t.max() * 1000:.1f}"
            )
            print(
                f"effective rate: {1 / t.mean():.2f} fps  ->  {mb / t.mean():.1f} MB/s"
            )
            print(
                f"read overhead (outside the exposure): "
...
def cmd_usb(args) -> None:
    ...
    tree = subprocess.run(
        ["ioreg", "-p", "IOUSB", "-w0"], capture_output=True, text=True
    ).stdout
    print(re.sub(r"<class.*", "", tree).rstrip() or "  (empty)")

    print("\n=== negotiated speed per device ===")
    devs = subprocess.run(
        ["ioreg", "-rc", "IOUSBHostDevice", "-w0", "-l"], capture_output=True, text=True
    ).stdout
    name = None
    hubs = []
...
def cmd_tec(args) -> None:
    with pick() as cam:
        ...
        cam.cooler = True
        print(f"target {cam.target_temperature:.1f} C — monitoring (ctrl-C to exit)")
        try:
            while True:
