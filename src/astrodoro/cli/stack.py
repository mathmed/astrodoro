...
from ..drivers import CameraDevice, CameraError, ImgType, open_camera
from ..settings import Settings


def setup(args) -> CameraDevice:
    try:
        cam = open_camera()
    ...
    if cam.id.port != "USB3.0":
        print(
            f"  warning: negotiated at {cam.id.port} — ~475 ms of dead time "
    ...
    if args.target_temp is not None and cam.supports_cooler:
        ...
        cam.cooler = True
        print(
            f"  cooler on, target {args.target_temp:+.1f} °C "
            f"(sensor at {cam.temperature:+.1f} °C now)"
        )

    print(
        f"  ROI {g.width}x{g.height} bin{g.bin} RAW16 | gain {cam.gain} | "
...
def capture_loop(cam: CameraDevice, q: queue.Queue, stop: threading.Event) -> None:
    cam.start_video()
    first = True
...
def grab(cam, n: int, report=None) -> list:
    ...
    for k in range(n + 1):
        ...
        if report is not None:
            ...
            print(
                f"  {len(frames)}/{n}  median={np.median(f):.0f} max={f.max()}",
                end="\r",
            )
    cam.stop_video()
    print()
...
def load_masters(args, shape=None) -> tuple:
    ...
    if getattr(args, "dark", None):
        ...
        if bias is not None:
            print(
                "  (the dark already contains the bias; the bias is not "
                "subtracted twice)"
            )
    ...
    if getattr(args, "flat", None):
        ...
        if flat is None:
            ...
            print(
                f"  flat: {args.flat} (corner at {flat.min() * 100:.0f}% of the centre)"
            )
    if shape is not None:
        for label, m in (("bias", bias), ("dark", dark), ("flat", flat)):
            if m is not None and m.shape != shape:
                sys.exit(
                    f"{label} is {m.shape}, the frames are {shape} (different bin?)"
                )
...
def cmd_bias(args) -> None:
    ...
    print(
        f"  median {np.median(master):.1f} ADU  sigma {master.std():.2f} ADU"
        f"  min {master.min():.0f}  max {master.max():.0f}"
    )
    if master.min() <= 0:
        print(
            "  warning: pixels at zero — the offset is truncating the left "
...
def cmd_dark(args) -> None:
    ...
    if cam.supports_cooler and args.target_temp is not None:
        ...
        while time.time() < deadline:
            sp, enable = ctrl.update(cam.temperature, cam.cooler_power, time.time())
            if sp is not None:
            ...
            if st.phase == "saturated":
                ...
                break
            time.sleep(3)
        print(f"\n  sensor at {cam.temperature:+.1f} °C, TEC {cam.cooler_power}%")

    print(f"\nCAP THE SENSOR. capturing {args.frames} darks of {cam.exposure:.1f}s...")
    input("Enter when it is capped: ")
    ...
    master = masters.combine(frames)
    path = masters.write(
        out, "dark", master, masters.Setup.from_camera(cam), len(frames)
    )
    print(f"master dark -> {path}")
    print(
        f"  median {np.median(master):.1f}  sigma {master.std():.1f}  "
        f"max {master.max():.0f}"
    )
...
def cmd_flat(args) -> None:
    ...
    if args.bias:
        ...
        print(
            "warning: without --bias or --dark, the offset pedestal is baked "
            "into the flat"
        )
    ...
    path = masters.write(
        out, "flat", master, masters.Setup.from_camera(cam), len(frames)
    )
    norm = master / m
    print(f"master flat -> {path}")
    print(f"  median {m:.0f} ({m / cam.full_scale * 100:.0f}% of scale)")
    print(
        f"  vignetting: corner at {norm.min() * 100:.0f}% of the centre, "
...
HEADER = (
    f"{'#':>4} {'stars':>5} {'FWHM':>6} {'pair':>4} {'rms':>5} "
...
def _row(st: LiveStacker, o, extra: str = "") -> str:
    ...
    dx, dy = st.drift()
    return (
        f"{st.n_stacked:4d} {o.n_stars:5d} {o.fwhm:6.2f} "
...
def cmd_run(args) -> None:
    ...
    for label, path in (("bias", args.bias), ("dark", args.dark), ("flat", args.flat)):
        if not path:
            continue
        try:
            for warning in masters.mismatch(
                label, read_fits(path)[1], masters.Setup.from_camera(cam)
            ):
    ...
    try:
        while True:
            ...
            rgb = (
                debayer.to_rgb(
                    (f * 65535).astype(np.uint16), cam.bayer, quality="linear"
                ).astype(np.float32)
                / 65535.0
            )
...
def cmd_replay(args) -> None:
    ...
    info = src.open()
    print(
        f"{info['name']}  {info['width']}x{info['height']} bin{info['bin']}  "
        f"Bayer {info['bayer']}  scale 0..{info['full_scale']}  "
        f"{info['n_frames']} frames"
    )

    bias, dark, flat, _hot = load_masters(args, shape=(info["height"], info["width"]))
    ...
    while True:
        ...
        rgb = (
            debayer.to_rgb(
                (f * 65535).astype(np.uint16), meta.bayer, quality="linear"
            ).astype(np.float32)
            / 65535.0
        )
        o = st.add(
            rgb, debayer.cfa_to_luminance(f), exposure=meta.exposure, lum_scale=2.0
        )
        print(_row(st, o))
    if st.n_stacked:
        _write_preview(st, out, args, final=True)
...
def _write_frame_preview(rgb: np.ndarray, out: Path, args) -> None:
    ...
    cv2.imwrite(
        str(out / "live.png"), cv2.cvtColor(stretch.to_uint8(disp), cv2.COLOR_RGB2BGR)
    )
...
def _write_preview(st: LiveStacker, out: Path, args, final: bool = False) -> None:
    ...
    cv2.imwrite(
        str(out / name), cv2.cvtColor(stretch.to_uint8(disp), cv2.COLOR_RGB2BGR)
    )
    if final:
...
def cmd_lucky(args) -> None:
    ...
    print(
        f"  sharpness {ranked[-1].sharpness:.1f} .. {ranked[0].sharpness:.1f}"
    ...
    if result.crop is not None:
        print(
            f"  stacked a {result.crop.w}x{result.crop.h} px window on the "
    ...
    cv2.imwrite(str(png), cv2.cvtColor(stretch.to_uint8(disp), cv2.COLOR_RGB2BGR))
    ...
    if result.background is not None:
        ...
        print(
            "  background removed: "
            + "  ".join(
                f"{c}={v * 100:.2f}%" for c, v in zip("RGB", pedestal, strict=False)
            )
        )
    print(
...
def cmd_sensor(args) -> None:
    ...
    for gain in gains:
        ...
        print(
            f"{gain:6d} {level:8.0f} {minimum:6.0f} {sigma:10.2f} "
            f"{norm:12.3f} {clip:6.2f}%"
        )

    if len(rows) >= 3:
        drops = [
            (rows[i][4] / rows[i - 1][4], rows[i - 1][0], rows[i][0])
        ...
        if ratio < 0.75:
            print(
                f"STEP detected between gain {before} and {after}: normalised "
            ...
            print(
                f"no clear step; the largest drop was {(1 - ratio) * 100:.0f}% "
    ...
    for off in [int(x) for x in args.offsets.split(",")]:
        ...
        if ok and best is None:
            best = off
        print(
            f"{off:7d} {level:8.0f} {minimum:6.0f} {zeros:8.4f}%  "
...
def add_parsers(sub) -> None:
    ...
    def common(sp):
        ...
        sp.add_argument("--exp", type=float, default=s.exposure_s, help="seconds")
    ...
    r.add_argument(
        "--bias",
        default=None,
        help="applied only when there is no --dark: a dark already contains the bias",
    )
    ...
    mn = sub.add_parser(
        "lucky", help="stack a Moon or planet burst (sharpest frames, aligned)"
    )
    mn.add_argument("folder", help="a burst folder written by PLANETS")
    mn.add_argument(
        "--best",
        type=float,
        default=lucky_stack_default(),
    ...
    mn.add_argument(
        "--out", default=None, help="default: stack_lucky.fits inside the burst folder"
    )
    mn.set_defaults(fn=cmd_lucky)

    sn = common(sub.add_parser("sensor", help="find the gain step and minimum offset"))
    sn.add_argument(
        "--gains", default="0,60,100,110,120,130,140,180,240,300,400,500,570"
    )
