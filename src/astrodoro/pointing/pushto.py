...
from ..core.iers import use_bundled_table
    ...
    if float(tgt.alt.deg) < 0:
        ...
        if abs(d_az) > tol / 2:
            bits.append(
                ...
                if d_az > 0
                else _("turn {amount} east").format(amount=_fmt(abs(d_az)))
            )
        text = " · ".join(bits) or _("{arcmin:.1f}' from the target").format(
            arcmin=sep * 60
        )
    if zenith and not on:
        text += _(" (near the zenith: azimuth is unstable, correct altitude first)")
    return Guidance(
        sep,
        d_alt,
        d_az,
