...
TYPE_LABELS = {
    "G": N_("galaxy"),
    "GPair": N_("galaxy pair"),
    "GTrpl": N_("galaxy triplet"),
    "GGroup": N_("galaxy group"),
    "GCl": N_("globular cluster"),
    "OCl": N_("open cluster"),
    "Cl+N": N_("cluster + nebula"),
    "PN": N_("planetary nebula"),
    "HII": N_("HII region"),
    "Neb": N_("nebula"),
    "RfN": N_("reflection nebula"),
    "EmN": N_("emission nebula"),
    "SNR": N_("supernova remnant"),
    "Dup": N_("duplicate"),
    "*": N_("star"),
    "**": N_("double star"),
    "*Ass": N_("stellar association"),
    "Nova": N_("nova"),
    "NonEx": N_("nonexistent"),
    "Other": N_("other"),
    "Moon": N_("Moon"),
    "Planet": N_("planet"),
}
...
class Catalog:
    ...
    def load(self, only_interesting: bool = True) -> Catalog:
        if not self.path.exists():
            raise FileNotFoundError(
                _("{path} not found — run `astrodoro catalog`").format(path=self.path)
            )
        rows: list[Obj] = []
            ...
            for line in fh:
                ...
                rows.append(
                    Obj(
                        ...
                        ra=ra,
                        dec=dec,
                        major_arcmin=num("MajAx"),
                        minor_arcmin=num("MinAx"),
    ...
    def near(
        self, ra: float, dec: float, radius_deg: float, limit: int = 40
    ) -> list[Obj]:
