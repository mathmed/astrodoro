...
class CameraSource:
    ...
    def __init__(
        ...
        self.cam: CameraDevice | None = None
        self._n = 0
        self._first = True

    def open(self) -> dict:
        try:
            cam = open_camera(self.index)
        except CameraError as e:
            raise RuntimeError(
                _("no camera found (is another capture program holding it?)")
            ) from e
        self.cam = cam
        cam.image_type = ImgType.RAW16
        g = cam.set_roi(bin=int(self._cfg["bin"]))
        cam.gain = int(self._cfg["gain"])
        cam.exposure = float(self._cfg["exposure"])
        cam.offset = int(self._cfg["offset"])
        return {
            "name": cam.id.name,
            ...
            "height": g.height,
            "bin": g.bin,
            "bayer": cam.bayer.name,
            "full_scale": cam.full_scale,
            "cooler": cam.supports_cooler,
            "forced": cam.forced_linear,
            "gain_range": cam.gain_range,
            "pixel_um": cam.pixel_size_um,
            "live": True,
        }

    @property
    def _open_cam(self) -> CameraDevice:
        """The camera, which only exists between `open()` and `close()`."""
        if self.cam is None:
            raise CameraError(_("the camera is not open"))
        return self.cam

    def start(self) -> None:
        self._open_cam.start_video()
        self._first = True
    ...
    @property
    def geometry(self):
        return self._open_cam.geometry
...
class ReplaySource:
    ...
    @property
    def geometry(self) -> Geometry:
        """From the file being replayed, which is what it has instead of a ROI."""
        return Geometry(
            x=0,
            y=0,
            width=int(self._info.get("width", 0)),
            height=int(self._info.get("height", 0)),
            bin=int(self._info.get("bin", 1)),
        )
    ...
    def apply(self, **kw) -> list[str]:
        ignored = [
            k
            for k in kw
            if k in ("gain", "exposure", "offset", "bin", "cooler", "target_temp")
        ]
        if ignored:
            return [
                _("in replay, {names} come from the file and cannot change").format(
                    names=", ".join(ignored)
                )
            ]
        return []
...
def _timestamp(h) -> float:
    ...
    try:
        ...
        return datetime.fromisoformat(str(v)).replace(tzinfo=UTC).timestamp()
    except Exception:
        return 0.0
