...
def estimate(
    ...
    try:
        ...
        al = _from_pairs(
            np.asarray(m_src), np.asarray(m_ref), min_matched, max_rms, "asterisms"
        )
        if al.ok:
            return al
        first = al.reason
    ...
    if al.ok:
        return al
    return _fail(
        _("{first}; voting: {second}").format(first=first, second=al.reason),
        al.n_matched,
    )
...
def _vote_translation(src_xy, ref_xy, tol, min_matched, max_rms) -> Alignment:
    ...
    if len(pairs_src) < 3:
        return _fail(_("not enough pairs after removing duplicates"), len(pairs_src))
    return _from_pairs(
        np.asarray(pairs_src),
        np.asarray(pairs_ref),
        min(min_matched, 3),
        max_rms,
        "vote",
    )
...
def _from_pairs(m_src, m_ref, min_matched, max_rms, method) -> Alignment:
    ...
    if keep.sum() < max(min_matched, 3):
        return _fail(_("only {n} inliers").format(n=int(keep.sum())), int(keep.sum()))
    ...
    if rms > max_rms:
        return _fail(
            _("rms {rms:.2f} px above the limit").format(rms=rms), int(keep.sum())
        )
