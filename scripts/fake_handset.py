...
#: pair is what caught the compass substitution bug.
...
def main() -> None:
    ...
    sock.errorOccurred.connect(
        lambda e: print(
            "error:", sock.errorString(), "— is the GUI's sensor switched on?"
        )
    )
        ...
        for line in sys.stdin:
            ...
            if c == "l":
                for i, (st, alt, az) in enumerate(visible[:15]):
                    print(f"  {i:2d}  {st.full:26s} alt {alt:5.1f}° az {az:6.1f}°")
