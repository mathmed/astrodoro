"""Binding ctypes de baixo nivel para o SVBony Camera SDK.

Mapeia SVBCameraSDK.h (API 3.0.0 / lib v1.13.4) praticamente 1:1.
Toda conveniencia fica em camera.py -- aqui e' so' a superficie C.

No modo C o header faz `#define SVB_CONTROL_TYPE int` (e o mesmo para
SVB_BOOL, SVB_ERROR_CODE, SVB_IMG_TYPE, SVB_BAYER_PATTERN), portanto todos
esses parametros trafegam como c_int.
"""
...
_VENDOR = Path(__file__).resolve().parent.parent / "vendor" / "lib"
# fallback: a dylib arm64 que acompanha o AstroDMx (mesma SDK)
...
def _load() -> C.CDLL:
    ...
    if os.environ.get("SVB_SDK_PATH"):
        cands.append(Path(os.environ["SVB_SDK_PATH"]))
    cands += [_VENDOR / _LIBNAME, _ASTRODMX / _LIBNAME]
    for p in cands:
        tried.append(str(p))
        if p.exists():
            try:
                ...
                tried[-1] += f"  ({e})"
    raise OSError(
        "libSVBCameraSDK.dylib nao encontrada/carregavel. Rode scripts/setup_sdk.sh.\n"
        + "\n".join("  tentei: " + t for t in tried)
    )
...
class Control(IntEnum):
    ...
    WB_B = 6
    FLIP = 7
    FRAME_SPEED_MODE = 8              # 0 baixa, 1 media, 2 alta
    CONTRAST = 9
    SHARPNESS = 10
    SATURATION = 11
    ...
    TARGET_TEMPERATURE = 15           # unidade 0.1 C
    CURRENT_TEMPERATURE = 16          # unidade 0.1 C
...
class ImgType(IntEnum):
    ...
    def bytes_per_pixel(self) -> int:
        ...
        if self is ImgType.RGB32:
            return 4
        return 2  # RAW10/12/14/16 e Y10..Y16 vem empacotados em 16 bits
...
class Bayer(IntEnum):
    ...
    def fits_name(self) -> str:
        """Padrao BAYERPAT do FITS, como Siril/PixInsight/ASTAP esperam."""
        return {Bayer.RG: "RGGB", Bayer.BG: "BGGR",
                Bayer.GR: "GRBG", Bayer.GB: "GBRG"}[self]

    @property
    def cv_code_rgb(self) -> str:
        """Nome do codigo OpenCV de demosaico. Cuidado: a nomenclatura do
        OpenCV e' deslocada em relacao ao padrao do sensor -- SVB_BAYER_RG
        (RGGB) corresponde a COLOR_BayerBG2RGB no OpenCV."""
        return {
...
class SVBError(RuntimeError):
    def __init__(self, op: str, code: int):
        ...
        super().__init__(f"{op} falhou: {ERRORS.get(code, '?')} ({code})")
...
class CameraProperty(C.Structure):
    _fields_ = [
        ...
        ("SupportedBins", C.c_int * 16),      # 0 encerra a lista
